...
function buildModalBlocks(type, item) {
  ...
  blocks.push({
    ...
    hint: { type: "plain_text", text: "공통, N2로 선택 시 해당 내용은 관리자페이지를 통해 N2담당자(방영권 부장님)께 전달됩니다./n단, 최종 등록 여부는 N2 매체사정에 따라 개별 확인이 필요한 점 양해 부탁드립니다." },
  ...
  blocks.push({
    ...
    hint: { type: "plain_text", text: "고객 피로도 조절 차원에서\n홈상단배너/관심그룹탭 배너는 최대 7일, 플로팅배너는 최대 3일만 가능합니다\n그 이상을 원하실 경우 추가로 배너를 등록해주세요"},
    element: {
      type: "datepicker",
      action_id: "end_date",
